...
class Role(models.Model):
	...
	def total_transcriptions(self, project=None):
		...
		transcription_set = []
		for token in self.transcription_tokens.filter(project__contract_client__name__contains=client_name, project__name__contains=project_name):
			for fragment in token.fragments.filter(is_reconciled=True):
				transcription_set.append(fragment.parent.id)

		return len(set(transcription_set))
